...
TEST_CASE("test function patt", "[mapping]") {
  SECTION("diagonal") {
    const int xdim = 3;
    auto patt = PattHelper::diag_patt(xdim);
  ...
  SECTION("extract") {
    ...
    SECTION("squared A") {
      ...
      // clang-format on
      SECTION("upper") {
        auto patt = PattHelper::extract_denseA_patt<MATRIX_FILL_MODE::UPPER>(A);
        REQUIRE(patt != nullptr);
        CHECK(patt->size() == 5);
      ...
      SECTION("full") {
        auto patt = PattHelper::extract_denseA_patt<MATRIX_FILL_MODE::FULL>(A);
        REQUIRE(patt != nullptr);
        CHECK(patt->size() == 7);
        CHECK(patt->count({0, 0}) == 1);
        CHECK(patt->count({0, 1}) == 1);
  ...
  SECTION("self_adjoint") {
    ...
    auto patt = PattHelper::extract_denseA_patt<MATRIX_FILL_MODE::FULL>(A);
    REQUIRE(patt != nullptr);
    auto symPatt = PattHelper::self_adjoint_patt(patt.get());
