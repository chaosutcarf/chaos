...
// template <int Xdim>
// struct auto_poly_f : public autodiff_function_base<auto_poly_f<Xdim>> {
...
// template <int Xdim>
// struct auto_vec_f : public autodiff_function_base<auto_vec_f<Xdim>> {
//   DECLARE_COMPILETIME_XDIM(Xdim);
...
//       f.ValJacHes(scalar_filler_t(val), gra_filler_t(jac), mat_filler_t(hes),
//                   x);
//       CHECK(numerical::near(val, scalar_expected::val(x)));
//       CHECK(std::equal_to<vecxr_t>()(jac, scalar_expected::jac(x)));
//       CHECK(std::equal_to<matxr_t>()(hes, scalar_expected::hes(x)));
...
//     SECTION("valjac") {
//       f.ValJac(scalar_filler_t(val), gra_filler_t(jac), x);
//       CHECK(numerical::near(val, scalar_expected::val(x)));
//       CHECK(std::equal_to<vecxr_t>()(jac, scalar_expected::jac(x)));
//     }
...
// template <>
// struct auto_poly_f<-1> : public autodiff_function_base<auto_poly_f<-1>> {
//   DECLARE_COMPILETIME_FDIM(1);
//   dim_t _Nx() const { return _xdim; }
...
// template <>
// struct auto_vec_f<-1> : public autodiff_function_base<auto_vec_f<-1>> {
//   dim_t _Nx() const { return _xdim; }
//   dim_t _Nf() const { return 2; }
