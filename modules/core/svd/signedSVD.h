...
class SignedSVD : Eigen::JacobiSVD<MatrixType> {
 protected:
  bool flipped_U, flipped_V, flipped_sigma;

  typename Eigen::JacobiSVD<MatrixType>::SingularValuesType
      singularValues_flipped;
  MatrixType matrixU_flipped, matrixV_flipped;

 public:
  SignedSVD(void) {}
  SignedSVD(const MatrixType& mtr, unsigned int computationOptions = 0) {
    compute(mtr, computationOptions);
  }

 public:
  template <int dim = MatrixType::RowsAtCompileTime>
  typename std::enable_if<dim == 3, SignedSVD<MatrixType>>::type& compute(
      const MatrixType& mtr, unsigned int computationOptions) {
  ...
  template <int dim = MatrixType::RowsAtCompileTime>
  typename std::enable_if<dim == 2, SignedSVD<MatrixType>>::type& compute(
      const MatrixType& mtr, unsigned int computationOptions) {
  ...
  void set(const Eigen::Matrix3d& U, const Eigen::Vector3d& Sigma,
 ...
 protected:
  void flip2d(const MatrixType& mtr, unsigned int computationOptions) {
    ...
    if (fullUComputed && fullVComputed) {
      if (Eigen::JacobiSVD<MatrixType>::m_matrixU.determinant() < 0.0) {
        ...
        if (!flipped_sigma) {
          singularValues_flipped =
              Eigen::JacobiSVD<MatrixType>::m_singularValues;
        }
  ...
  // 24 mults, 20 adds, 1 atan2, 1 sincos, 2 sqrts
  {
    ...
    T p = ((T)1 / 6) * (a00 * a00 + a11 * a11 + a22 * a22 +
                        2 * (a12_sqr + a13_sqr + a23_sqr));
    T q =
        (T).5 * (a00 * (a11 * a22 - a23_sqr) - a11 * a13_sqr - a22 * a12_sqr) +
        A_Sym(0, 1) * A_Sym(0, 2) * A_Sym(1, 2);
    T sqrt_p = sqrt(p);
    T disc = p * p * p - q * q;
  ...
  void fastEigenvectors(const Eigen::Matrix3d& A_Sym,
                        const Eigen::Vector3d& lambda, Eigen::Matrix3d& V)
  // 71 mults, 44 adds, 3 divs, 3 sqrts
  {
    ...
    Eigen::Matrix3d C1;
    computeCofactorMtr<3>(A_Sym - lambda_flip(0) * Eigen::Matrix3d::Identity(),
                          C1);
    Eigen::Matrix3d::Index i;
    T norm2 = C1.colwise().squaredNorm().maxCoeff(
        &i);  // 3a + 12m+6a + 9m+6a+1d+1s = 21m+15a+1d+1s
    Eigen::Vector3d v1;
    if (norm2 != 0) {
    ...
    Eigen::Vector3d v1_orthogonal =
        v1.unitOrthogonal();  // 6m+2a+1d+1s (tweak: 5m+1a+1d+1s)
    Eigen::Matrix<T, 3, 2> other_v;
    other_v.col(0) = v1_orthogonal;
    other_v.col(1) = v1.cross(v1_orthogonal);  // 6m+3a (tweak: 4m+1a)
    Eigen::Matrix2d A_reduced =
        other_v.transpose() * A_Sym * other_v;  // 21m+12a (tweak: 18m+9a)

    // find third eigenvector from A_reduced, and fill in second via cross
    // product
    Eigen::Matrix2d C3;
    computeCofactorMtr<2>(
        A_reduced - lambda_flip(2) * Eigen::Matrix2d::Identity(), C3);
    Eigen::Matrix2d::Index j;
    // 3a + 12m+6a + 9m+6a+1d+1s = 21m+15a+1d+1s
    norm2 = C3.colwise().squaredNorm().maxCoeff(&j);
    Eigen::Vector3d v3;
    if (norm2 != 0) {
      T one_over_sqrt = (T)1 / sqrt(norm2);
  ...
  void fastSolveEigenproblem(const Eigen::Matrix3d& A_Sym,
                             Eigen::Vector3d& lambda, Eigen::Matrix3d& V)
  // 71 mults, 44 adds, 3 divs, 3 sqrts
  {
  ...
  void fastSVD3d(const Eigen::Matrix3d& A, Eigen::Matrix3d& U,
                 Eigen::Vector3d& singular_values, Eigen::Matrix3d& V)
 ...
 public:
  const typename Eigen::JacobiSVD<MatrixType>::SingularValuesType&
  singularValues(void) const {
    if (flipped_sigma) {
