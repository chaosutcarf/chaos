...
//-> based on
//   https://www.researchgate.net/publication/263580188_Closed_Form_SVD_Solutions_for_2_x_2_Matrices_-_Rev_2
template <typename MatrixType>
class ClosedFormSVD2d : Eigen::JacobiSVD<MatrixType> {
  typedef Eigen::JacobiSVD<MatrixType> Base;

 public:
  ClosedFormSVD2d(void) {}
  ClosedFormSVD2d(const MatrixType& mtr, unsigned int computationOptions = 0)
 ...
 public:
  ClosedFormSVD2d& compute(const MatrixType& A) {
    ...
    if (dif <= 0.0) {
      ...
      Base::m_singularValues[1] =
          ((_2admbc < 0.0) ? (-std::abs(sqrt_sum - sqrt_dif) / 2.0)
                           : (std::abs(sqrt_sum - sqrt_dif) / 2.0));
      ...
      if (computeU || computeV) {
        ...
        if (computeU) {
          ...
          const double sinl =
              ((sinl2 <= 0.0)
                   ? 0.0
                   : (neg_ab_p_cd ? -std::sqrt(sinl2) : std::sqrt(sinl2)));
        ...
        if (computeV) {
          ...
          const double sinr =
              ((sinr2 <= 0.0)
                   ? 0.0
                   : (neg_ac_p_bd ? -std::sqrt(sinr2) : std::sqrt(sinr2)));
  ...
  ClosedFormSVD2d& compute(const MatrixType& mtr,
 ...
 protected:
  void allocate(unsigned int computationOptions) {
    if (Base::m_isAllocated && 2 == Base::m_rows && 2 == Base::m_cols &&
    ...
    eigen_assert(
        EIGEN_IMPLIES(Base::m_computeThinU || Base::m_computeThinV,
 ...
 public:
  ...
  const MatrixType& matrixU(void) const { return Base::m_matrixU; }
  const MatrixType& matrixV(void) const { return Base::m_matrixV; }
};
}  // namespace chaos::math
