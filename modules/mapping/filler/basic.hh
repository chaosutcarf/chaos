...
struct DenseMatrixFillerCore {
  ...
  inline void setZero() { m_data.setZero(); }
  inline index_t rows() const { m_data.rows(); }
  inline index_t cols() const { m_data.cols(); }
  template <bool isOverride>
...
struct COOFillerCore {
  ...
  static auto create(T &data, index_t rows, index_t cols) {
